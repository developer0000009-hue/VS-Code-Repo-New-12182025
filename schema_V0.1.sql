...
CREATE TABLE IF NOT EXISTS public.school_branches (
    ...
    school_user_id uuid REFERENCES public.profiles(id) ON DELETE CASCADE ON UPDATE CASCADE,
    name text NOT NULL,
    address text,
    city text,
    state text,
    ...
    admin_phone text,
    admin_email text,
    branch_admin_id uuid REFERENCES public.profiles(id) ON DELETE SET NULL ON UPDATE CASCADE,
    created_at timestamptz DEFAULT now() NOT NULL
);
...
BEGIN
    ...
    JOIN public.admissions a ON sc.admission_id = a.id
...
CREATE OR REPLACE FUNCTION public.verify_and_link_branch_admin() RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER AS $$ BEGIN RETURN jsonb_build_object('success', true); END; $$;
