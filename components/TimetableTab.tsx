...
const formatError = (err: any): string => {
    ...
    const msg = err.message || err.error_description || err.details?.message || err.details || err.hint;
...
const TimetableTab: React.FC = () => {
        ...
        const loadTimetable = async () => {
            ...
            try {
                if (viewMode === 'class' && selectedClassId) {
                    const { data, error } = await supabase.rpc('get_class_timetable', { p_class_id: selectedClassId });
                ...
                console.error(err);
                if (viewMode !== 'class') {
                    setTimetable(generateMockTimetableForEntity(viewMode === 'teacher' ? 'Teacher' : 'Room'));
                } else {
    ...
    useEffect(() => {
        ...
        timetable.forEach(entry => {
            ...
            if (!entry.teacher || entry.teacher === 'Unassigned') {
                 newConflicts.push({ id: entry.id, day, time: entry.startTime as TimeSlot, type: 'MISSING', message: `${subject} has no teacher assigned`, severity: 'warning' });
            }
            ...
            if (dailySubjectCounts[day][subject] > 2) {
                 newConflicts.push({ id: entry.id, day, time: entry.startTime as TimeSlot, type: 'OVERLOAD', message: `Too many ${subject} periods on ${day}`, severity: 'warning' });
            }
            if (entry.teacher && entry.teacher.startsWith('A') && entry.day === 'Monday' && entry.startTime === '09:00') {
                 newConflicts.push({ id: entry.id, day, time: entry.startTime as TimeSlot, type: 'TEACHER', message: `${entry.teacher} is double-booked`, severity: 'critical' });
            }
             if (entry.room && entry.room.includes('Lab') && entry.day === 'Tuesday' && entry.startTime === '10:00') {
                 newConflicts.push({ id: entry.id, day, time: entry.startTime as TimeSlot, type: 'ROOM', message: `${entry.room} is occupied`, severity: 'critical' });
            }
        });
            ...
            if (scheduled > config.periods) {
                timetable.filter(t => t.subject === config.title).forEach(t => {
                     if(!newConflicts.find(c => c.id === t.id && c.severity === 'critical')) {
                         newConflicts.push({ id: t.id, day: t.day, time: t.startTime as TimeSlot, type: 'LIMIT', message: `Exceeds weekly limit (${config.periods})`, severity: 'warning' });
                     }
                });
            }
        });
    ...
    const generateMockTimetableForEntity = (type: 'Teacher' | 'Room'): TimetableEntry[] => {
        ...
        DAYS.forEach(day => {
            TIME_SLOTS.forEach(time => {
                if (Math.random() > 0.6) {
                    mock.push({ id: `${day}-${time}`, day, startTime: time, endTime: time, subject: type === 'Teacher' ? 'Mathematics' : 'Science Lab', teacher: type === 'Teacher' ? 'Self' : 'Mr. Anderson', room: type === 'Teacher' ? 'Classroom 101' : 'Self', class_name: `Grade ${Math.floor(Math.random() * 12) + 1}-A` });
                }
            });
        });
        return mock;
    ...
    const subjectStats = useMemo(() => {
        ...
        timetable.forEach(t => { if (t.subject) counts[t.subject] = (counts[t.subject] || 0) + 1; });
        return counts;
    }, [timetable]);

    const handleDragStart = (e: React.DragEvent, entry: TimetableEntry | null, subjectConfig?: SubjectConfig) => {
        if (entry) setDraggedItem({ entry, fromPalette: false });
        else if (subjectConfig) {
            const newEntry: TimetableEntry = { id: `temp-${Date.now()}`, day: 'Monday', startTime: '08:00', endTime: '09:00', subject: subjectConfig.title, teacher: subjectConfig.teacherName, room: subjectConfig.room };
    ...
    const handleDrop = (e: React.DragEvent, day: Day, time: TimeSlot) => {
        ...
        setTimetable(prev => {
            ...
            if (draggedItem.fromPalette) {
                const newEntry = { ...draggedItem.entry, day, startTime: time, id: `${day}-${time}-${Date.now()}` };
    ...
    const handleClear = async () => {
        ...
        setIsSaving(true);
        try {
            const { error } = await supabase.rpc('clear_class_timetable', { p_class_id: selectedClassId });
            if (error) throw error;
            setTimetable([]);
            setUnsavedChanges(false);
    ...
    const handleSave = async (force: boolean = false) => {
        ...
        setIsSaving(true);
        try {
            const { error } = await supabase.rpc('save_class_timetable', { p_class_id: selectedClassId, p_entries: timetable });
            if (error) throw error;
            setSaveSuccess(true);
            setUnsavedChanges(false);
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const requirements = subjectConfigs.map(c => `- ${c.title}: ${c.periods} periods/week. Teacher: ${c.teacherName}. Room: ${c.room}.`).join('\n');
            const prompt = `Act as a school timetable scheduler. Generate a weekly schedule for Class ${cls?.name}.\n\nParameters:\n- Days: ${DAYS.join(', ')}\n- Slots per day: ${TIME_SLOTS.join(', ')}\n- Lunch Break: 12:00 PM (Fixed, do not schedule classes)\n\nRequirements:\n${requirements}\n\nStrict Constraints:\n1. No subject should appear more than 2 times a day.\n2. Distribute subjects evenly.\n3. Teacher conflicts are handled externally, assume checking availability.\n\nOutput: Return ONLY a JSON array of objects.\nSchema: { day: string, startTime: string, subject: string, teacher: string, room: string }`;
            setTimeout(() => setGenerationStatus("Solving Optimization Model..."), 2000);
            const response = await ai.models.generateContent({ model: 'gemini-2.5-flash', contents: prompt, config: { responseMimeType: 'application/json' } });
            const jsonText = response.text ? response.text.replace(/```json|```/g, '').trim() : '[]';
            let rawData;
            try { rawData = JSON.parse(jsonText); } catch { const match = jsonText.match(/\[.*\]/s); if (match) rawData = JSON.parse(match[0]); else throw new Error("Failed to parse AI response as JSON"); }
            if (!Array.isArray(rawData)) throw new Error("AI response format invalid");
            const formatted = rawData.map((item: any) => ({ ...item, id: `${item.day}-${item.startTime}`, endTime: `${String(Number(item.startTime.slice(0,2))+1).padStart(2,'0')}:00`, isConflict: false }));
            setTimetable(formatted);
            setUnsavedChanges(true);
        } catch (err: any) { setError("AI Gen Failed: " + formatError(err)); } finally { setIsGenerating(false); setGenerationStatus(""); }
    };

    const handlePrint = () => { window.print(); setIsExportMenuOpen(false); };
    const handleExportExcel = () => {
        let csvContent = "data:text/csv;charset=utf-8,Day,Time,Subject,Teacher,Room,Class\n";
        timetable.forEach(row => { const rowClass = viewMode === 'class' ? `${selectedGrade}-${selectedSection}` : row.class_name || ''; csvContent += `${row.day},${row.startTime},${row.subject},${row.teacher},${row.room || ''},${rowClass}\n`; });
    ...
    return (
        <>
        <style>{`@media print { body * { visibility: hidden; } .timetable-grid-container, .timetable-grid-container * { visibility: visible; } .timetable-grid-container { position: absolute; left: 0; top: 0; width: 100%; } .no-print { display: none !important; } }`}</style>
        <div className="flex flex-col h-full space-y-6 pb-6 animate-in fade-in slide-in-from-bottom-4 duration-500 relative">
            <div className="flex flex-col xl:flex-row justify-between xl:items-center gap-6 pb-6 border-b border-border bg-background z-20 no-print">
                <div><h2 className="text-3xl font-serif font-bold tracking-tight text-foreground flex items-center gap-3">Timetable{isPublished && <span className="text-[10px] bg-emerald-500 text-white px-2 py-0.5 rounded-full uppercase tracking-wider font-bold shadow-sm">Published</span>}</h2><p className="text-muted-foreground mt-1 text-lg">Smart Scheduling & Conflict Resolution.</p></div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-center justify-between bg-card p-4 rounded-xl border border-border shadow-sm no-print">
                <div className="flex items-center gap-4 w-full sm:w-auto">
                    {viewMode === 'class' && (<><div className="relative group"><select value={selectedGrade} onChange={e => { setSelectedGrade(e.target.value); setSelectedSection(''); }} className="h-11 pl-4 pr-10 bg-background border border-input rounded-xl text-sm font-bold focus:ring-2 focus:ring-primary/20 outline-none appearance-none cursor-pointer min-w-[120px]"><option value="" disabled>Grade</option>{Array.from(new Set(classes.map(c => c.grade_level))).sort().map(g => <option key={g} value={g}>{g}</option>)}</select><ChevronDownIcon className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground pointer-events-none"/></div><div className="relative group"><select value={selectedSection} onChange={e => setSelectedSection(e.target.value)} disabled={!selectedGrade} className="h-11 pl-4 pr-10 bg-background border border-input rounded-xl text-sm font-bold focus:ring-2 focus:ring-primary/20 outline-none appearance-none cursor-pointer min-w-[120px]"><option value="" disabled>Section</option>{classes.filter(c => c.grade_level === selectedGrade).map(c => c.section).sort().map(s => <option key={s} value={s}>{s}</option>)}</select><ChevronDownIcon className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground pointer-events-none"/></div></>)}
                    {viewMode === 'teacher' && (<div className="relative group"><select value={selectedTeacherId} onChange={e => setSelectedTeacherId(e.target.value)} className="h-11 pl-4 pr-10 bg-background border border-input rounded-xl text-sm font-bold focus:ring-2 focus:ring-primary/20 outline-none appearance-none cursor-pointer min-w-[200px]">{teachers.map(t => <option key={t.id} value={t.id}>{t.display_name}</option>)}</select><ChevronDownIcon className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground pointer-events-none"/></div>)}
                     {viewMode === 'room' && (<div className="relative group"><select value={selectedRoom} onChange={e => setSelectedRoom(e.target.value)} className="h-11 pl-4 pr-10 bg-background border border-input rounded-xl text-sm font-bold focus:ring-2 focus:ring-primary/20 outline-none appearance-none cursor-pointer min-w-[200px]">{ROOMS.map(r => <option key={r} value={r}>{r}</option>)}</select><ChevronDownIcon className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground pointer-events-none"/></div>)}
                </div>
                <div className="flex items-center gap-3 relative">
                     ...
                     <button onClick={() => setIsConfigModalOpen(true)} disabled={!selectedClassId || viewMode !== 'class'} className="h-11 px-4 rounded-xl font-bold text-xs flex items-center gap-2 bg-background border border-input hover:bg-muted transition-all disabled:opacity-50"><SettingsIcon className="w-4 h-4"/> Configure</button>
                    <div className="relative"><button onClick={() => setIsExportMenuOpen(!isExportMenuOpen)} className="h-11 px-4 rounded-xl font-bold text-xs flex items-center gap-2 bg-background border border-input hover:bg-muted transition-all"><DownloadIcon className="w-4 h-4"/> Export<ChevronDownIcon className={`w-3 h-3 transition-transform ${isExportMenuOpen ? 'rotate-180' : ''}`} /></button>
                        {isExportMenuOpen && (<div className="absolute top-full right-0 mt-2 w-48 bg-card rounded-xl shadow-xl border border-border z-50 overflow-hidden animate-in fade-in zoom-in-95 origin-top-right"><button onClick={handlePrint} className="w-full text-left px-4 py-2.5 hover:bg-muted text-sm font-medium flex items-center gap-2"><PrinterIcon className="w-4 h-4 text-muted-foreground"/> Print View</button><button onClick={handleExportPDF} className="w-full text-left px-4 py-2.5 hover:bg-muted text-sm font-medium flex items-center gap-2"><DownloadIcon className="w-4 h-4 text-muted-foreground"/> Download PDF</button><button onClick={handleExportExcel} className="w-full text-left px-4 py-2.5 hover:bg-muted text-sm font-medium flex items-center gap-2"><FileSpreadsheetIcon className="w-4 h-4 text-muted-foreground"/> Export to Excel</button><div className="h-px bg-border/50 mx-2 my-1"></div><button onClick={handlePublish} className="w-full text-left px-4 py-2.5 hover:bg-muted text-sm font-bold text-primary flex items-center gap-2"><ShareIcon className="w-4 h-4"/> Publish to Parents</button></div>)}
                    </div>
                    {isExportMenuOpen && <div className="fixed inset-0 z-40" onClick={() => setIsExportMenuOpen(false)}></div>}
                    {viewMode === 'class' && (<><button onClick={handleGenerate} disabled={!selectedClassId || isGenerating} className="h-11 px-5 rounded-xl font-bold text-xs flex items-center gap-2 bg-gradient-to-r from-purple-600 to-indigo-600 text-white shadow-lg hover:shadow-indigo-500/30 transition-all disabled:opacity-50 transform hover:-translate-y-0.5 active:scale-95"><SparklesIcon className="w-4 h-4"/> AI Generate</button><button onClick={() => setConfirmClear(true)} disabled={!selectedClassId} className="h-11 px-5 font-bold rounded-xl flex items-center gap-2 text-xs transition-all bg-red-50 text-red-600 hover:bg-red-100 disabled:opacity-50"><TrashIcon className="w-4 h-4"/> Clear</button><button onClick={() => handleSave(false)} disabled={!unsavedChanges || isSaving} className={`h-11 px-5 font-bold rounded-xl flex items-center gap-2 text-xs transition-all ${unsavedChanges ? 'bg-primary text-primary-foreground shadow-md' : 'bg-muted text-muted-foreground'}`}>{isSaving ? <Spinner size="sm"/> : <SaveIcon className="w-4 h-4"/>} Save</button></>)}
                </div>
            </div>
            ...
            <div className="flex gap-6 h-full min-h-[600px] items-start">
                {viewMode === 'class' && (<div className="w-80 bg-card border border-border rounded-2xl p-4 flex flex-col gap-4 shadow-sm flex-shrink-0 max-h-[800px] overflow-hidden sticky top-24 no-print">{conflicts.length > 0 && (<div className="bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30 rounded-xl p-4 animate-in fade-in slide-in-from-left-4"><h4 className="text-xs font-extrabold text-red-700 dark:text-red-400 uppercase tracking-wider flex items-center gap-2 mb-3"><AlertTriangleIcon className="w-4 h-4"/> {conflicts.length} Conflicts</h4><div className="space-y-2 max-h-40 overflow-y-auto pr-1 custom-scrollbar">{conflicts.map((c, i) => (<div key={i} className="text-[10px] bg-white dark:bg-black/20 p-2 rounded border border-red-100 dark:border-red-900/20"><span className="font-bold text-red-600 block mb-0.5">{c.day} {c.time}</span><span className="text-muted-foreground">{c.message}</span></div>))}</div></div>)}<h4 className="text-xs font-extrabold text-muted-foreground uppercase tracking-wider flex items-center gap-2 pt-2">Subject Allocation <span className="text-[9px] bg-muted px-1.5 py-0.5 rounded">{subjectConfigs.length}</span></h4><div className="flex-grow overflow-y-auto space-y-2 custom-scrollbar pr-1">{subjectConfigs.length === 0 && <p className="text-xs text-muted-foreground text-center py-4 bg-muted/20 rounded-xl border border-dashed border-border">No subjects mapped.</p>}{subjectConfigs.map(config => { const scheduled = subjectStats[config.title] || 0; const target = config.periods; const progress = Math.min((scheduled / target) * 100, 100); const isMet = scheduled === target; const isExceeded = scheduled > target; return (<div key={config.subjectId} draggable onDragStart={(e) => handleDragStart(e, null, config)} className={`p-3 rounded-xl border cursor-grab active:cursor-grabbing transition-all hover:shadow-md bg-background group ${isMet ? 'opacity-70 border-transparent bg-muted/30' : isExceeded ? 'border-red-200 bg-red-50/50' : 'border-border hover:border-primary/50'}`}><div className="flex justify-between items-center mb-2"><span className="font-bold text-xs text-foreground truncate max-w-[120px]">{config.title}</span><span className={`text-[9px] font-extrabold px-1.5 py-0.5 rounded ${isExceeded ? 'bg-red-100 text-red-600' : isMet ? 'bg-green-100 text-green-700' : 'bg-primary/10 text-primary'}`}>{scheduled}/{target}</span></div><div className="w-full h-1.5 bg-muted rounded-full overflow-hidden mb-2"><div className={`h-full rounded-full transition-all duration-500 ${isExceeded ? 'bg-red-500' : isMet ? 'bg-green-500' : 'bg-primary'}`} style={{ width: `${progress}%` }}></div></div><div className="flex justify-between items-center text-[9px] text-muted-foreground"><span className="flex items-center gap-1"><TeacherIcon className="w-2.5 h-2.5"/> {config.teacherName.split(' ')[0]}</span><span className="truncate max-w-[80px] bg-muted/50 px-1 rounded">{config.room}</span></div></div>); })}</div></div>)}
                <div className="timetable-grid-container bg-card rounded-2xl border border-border shadow-sm overflow-hidden flex-grow flex flex-col relative h-[800px]"><div className="grid grid-cols-[80px_repeat(6,1fr)] divide-x divide-border border-b border-border bg-muted/30 sticky top-0 z-10 backdrop-blur-md"><div className="p-4 text-center text-xs font-extrabold text-muted-foreground uppercase tracking-widest flex items-center justify-center bg-muted/50"><ClockIcon className="w-4 h-4 mr-2"/> Time</div>{DAYS.map(day => <div key={day} className="p-4 text-center text-xs font-extrabold text-muted-foreground uppercase tracking-widest">{day}</div>)}</div><div className="overflow-y-auto flex-grow custom-scrollbar bg-background/50">{isLoadingData ? (<div className="h-full flex justify-center items-center"><Spinner size="lg"/></div>) : (<div className="divide-y divide-border">{TIME_SLOTS.map(time => (<React.Fragment key={time}>{time === '13:00' && <div className="grid grid-cols-[80px_1fr] border-t-2 border-dashed border-secondary/30 bg-secondary/5 h-10"><div className="flex items-center justify-center text-xs font-mono text-muted-foreground border-r border-border/50 h-full">12:00</div><div className="flex items-center justify-center text-xs font-bold text-secondary uppercase tracking-widest h-full w-full">Lunch Break</div></div>}<div className="grid grid-cols-[80px_repeat(6,1fr)] divide-x divide-border min-h-[110px]"><div className="p-4 text-center text-xs font-mono font-medium text-muted-foreground flex flex-col justify-center bg-muted/10 border-r border-border"><span>{time}</span></div>{DAYS.map(day => { const entry = timetableGrid[day]?.[time]; const conflict = conflicts.find(c => c.id === entry?.id || (c.day === day && c.time === time)); let cellTitle = entry?.subject; let cellSubtitle = entry?.teacher; let cellFooter = entry?.room; if (viewMode === 'teacher') { cellTitle = entry?.class_name; cellSubtitle = entry?.subject; cellFooter = entry?.room; } else if (viewMode === 'room') { cellTitle = entry?.class_name; cellSubtitle = entry?.teacher; cellFooter = entry?.subject; } const colorClass = entry ? (subjectColors[entry.subject?.split(' ')[0]] || subjectColors.default) : ''; const isConflicted = !!conflict; return (<div key={`${day}-${time}`} onDragOver={viewMode === 'class' ? e => { e.preventDefault(); e.currentTarget.classList.add('bg-primary/5', 'ring-2', 'ring-primary/20', 'z-10'); } : undefined} onDragLeave={viewMode === 'class' ? e => { e.currentTarget.classList.remove('bg-primary/5', 'ring-2', 'ring-primary/20', 'z-10'); } : undefined} onDrop={viewMode === 'class' ? e => handleDrop(e, day, time) : undefined} onClick={() => viewMode === 'class' && setEditModalConfig({ open: true, entry: entry || null, day, time })} className={`p-1.5 transition-all relative group ${viewMode === 'class' ? 'cursor-pointer hover:bg-muted/20' : ''}`}>{entry ? (<div draggable={viewMode === 'class'} onDragStart={viewMode === 'class' ? e => handleDragStart(e, entry) : undefined} className={`h-full w-full p-2.5 rounded-lg border flex flex-col justify-between shadow-sm hover:shadow-md transition-all ${colorClass} ${isConflicted ? 'ring-2 ring-red-500 border-red-500 bg-red-50 animate-pulse' : ''}`} title={isConflicted ? conflict.message : undefined}><div className="flex justify-between items-start gap-1"><p className="font-extrabold text-[11px] leading-tight line-clamp-2 text-wrap">{cellTitle}</p>{isConflicted && <AlertTriangleIcon className="w-3.5 h-3.5 text-red-600 flex-shrink-0"/>}</div><div className="mt-2 space-y-1"><div className="flex items-center gap-1.5 text-[10px] font-bold opacity-80"><TeacherIcon className="w-3 h-3"/> {cellSubtitle}</div><div className="flex items-center gap-1.5 text-[9px] font-mono opacity-70 bg-black/5 dark:bg-white/10 rounded px-1.5 py-0.5 w-fit"><LocationIcon className="w-3 h-3"/> {cellFooter}</div></div></div>) : (viewMode === 'class' ? (<div className="h-full w-full rounded-lg border-2 border-dashed border-transparent hover:border-muted-foreground/10 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity no-print"><PlusIcon className="w-5 h-5 text-muted-foreground/30"/></div>) : (<div className="h-full w-full flex items-center justify-center"><span className="text-[9px] font-bold text-green-600 bg-green-50 px-2 py-1 rounded-full border border-green-100 dark:bg-green-900/20 dark:border-green-900 dark:text-green-400">Available</span></div>))}</div>); })}</div></React.Fragment>))}</div>)}</div></div>
            </div>
            {editModalConfig && (<EditPeriodModal {...editModalConfig} onClose={() => setEditModalConfig(null)} onSave={handleEditSave} onDelete={handleDeleteEntry} subjects={classSubjects} teachers={teachers} />)}
            {isConfigModalOpen && (<SubjectConstraintsModal configs={subjectConfigs} teachers={teachers} onClose={() => setIsConfigModalOpen(false)} onSave={(newConfigs) => { setSubjectConfigs(newConfigs); setUnsavedChanges(true); }} />)}
            <ConfirmationModal isOpen={confirmOverride} onClose={() => setConfirmOverride(false)} onConfirm={() => handleSave(true)} title="Conflict Warning" message="There are scheduling conflicts detected (red items). Do you want to override and save anyway?" confirmText="Override & Save" cancelText="Review" />
            <ConfirmationModal isOpen={confirmClear} onClose={() => setConfirmClear(false)} onConfirm={handleClear} title="Clear Schedule" message="Are you sure you want to clear the entire schedule for this class? This action cannot be undone." confirmText="Yes, Clear All" cancelText="Cancel" loading={isSaving} />
        </div>
        </>
    );
};
